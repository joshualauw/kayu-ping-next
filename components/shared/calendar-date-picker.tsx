...
export const CalendarDatePicker = React.forwardRef<HTMLButtonElement, CalendarDatePickerProps>(
  (
    ...
    const toDate = (date: Date, options?: { timeZone: string }) => {
          ...
          const from =
            numberOfMonths === 2
              ...
              : date?.from
                ...
                : newMonth;
          const to =
            numberOfMonths === 2 ? (date.to ? endOfDay(toDate(date.to, { timeZone })) : endOfMonth(toDate(newMonth, { timeZone }))) : from;
          if (from <= to) {
            onDateSelect({ from, to });
    ...
    const formatWithTz = (date: Date, fmt: string) => {
      const dayjsFmt = fmt === "dd" ? "DD" : fmt === "LLL" ? "MMM" : fmt === "y" ? "YYYY" : fmt;
      return dayjs(date).tz(timeZone).format(dayjsFmt);
    };
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-4">
                    <div className="ml-3 flex gap-2">
                      ...
                      >
                        <SelectTrigger className="w-30 font-medium hover:bg-accent hover:text-accent-foreground focus:ring-0 focus:ring-offset-0">
