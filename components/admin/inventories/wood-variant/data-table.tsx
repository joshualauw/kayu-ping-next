...
import { useGetAllInventoriesByWoodVariant } from "@/hooks/swr/inventories/use-get-all-inventories-by-wood-variant";
import type { InventoryGroupedByVariantItem } from "@/lib/services/inventory-service";
...
export default function WoodVariantInventoriesDataTable() {
  ...
  const columns = useMemo<ColumnDef<InventoryGroupedByVariantItem>[]>(
    () => [
      {
        id: "expander",
        header: () => null,
  ...
  const renderRowDetails = (row: Row<InventoryGroupedByVariantItem>) => {
    const items = row.original.items;
    const variantVolume = row.original.variant.volume;
    return (
                ...
                >
                  ...
                  <td className="h-9 px-3 align-middle font-mono text-[10px]">
                    <div className="space-y-0.5">
                      <div>Single: {variantVolume.toFixed(4)}</div>
